-- ============================================================
-- INFINITE ZEN - MÓDULO BLOX STRIKE v1.0
...
function BloxStrike.Init(ctx)
    ...
    local FeatureLabels = {
        ...
        noRecoil = "No-Recoil", rapidFire = "Rapid Fire",
    ...
    local function getTargetPart(p)
        ...
        elseif mode == 2 then
            return getBasePart(p.Character, "UpperTorso", "Torso")
        else
    ...
    -- TEAM BADGE
    local TeamBadge = Instance.new("TextLabel", Header)
    TeamBadge.Size = UDim2.new(0, 120, 0, 22); TeamBadge.Position = UDim2.new(1, -240, 0.5, -11)
    TeamBadge.BackgroundColor3 = Theme.Surface2
    TeamBadge.Font = Theme.FontBold; TeamBadge.TextSize = 11
    TeamBadge.TextColor3 = Color3.fromRGB(100, 200, 255)
    TeamBadge.Text = "❓ Sem time"
    TeamBadge.ZIndex = 3
    Instance.new("UICorner", TeamBadge).CornerRadius = UDim.new(0, 6)
    ...
    task.spawn(function()
        while not UNLOADED do
            ...
            if team == "Counter-Terrorists" then
                TeamBadge.Text = "🛡 CT"
                TeamBadge.TextColor3 = Color3.fromRGB(100, 180, 255)
            elseif team == "Terrorists" then
                TeamBadge.Text = "💣 T"
                TeamBadge.TextColor3 = Color3.fromRGB(255, 150, 80)
            elseif team then
                TeamBadge.Text = team
                TeamBadge.TextColor3 = Theme.Text
            else
                TeamBadge.Text = "👁 Spectator"
                TeamBadge.TextColor3 = Theme.TextDim
            end
            task.wait(0.5)
        end
    end)
    ...
    task.spawn(function()
        while not UNLOADED do
            local myHealth = LocalPlayer:GetAttribute("Health")
            if myHealth and lastHealth and myHealth < lastHealth then
                lastDamageTime = tick()
            end
            if myHealth then lastHealth = myHealth end
            task.wait(0.1)
        end
    end)
    ...
    local function updateESP(p, char)
        ...
        if headOn and footOn then
            ...
            local maxHP = 100
            local hr = math.clamp(hp / maxHP, 0, 1)
    ...
    local function loadConfigNamed(name)
        ...
        if not State.headExpander then restoreAll() end
    ...
    CreditsTab.CreateLabel("Blox Strike Edition", Theme.Warning)
    ...
    print("[Infinite Zen] Configs em: InfiniteZen_Configs/BloxStrike")
end
